...
- **Intake Review Queue** (`src/features/dashboard/components/IntakeReviewQueue.jsx`) renders a high-priority inbox on the dashboard for students flagged with `needs_intake_approval`, with a one-click approval action wired to `/api/intake/approve`. It shows the configured important fields and allows expanding to the full intake.
