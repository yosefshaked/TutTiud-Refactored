...
- **Intake Review Queue** (`src/features/dashboard/components/IntakeReviewQueue.jsx`) renders a high-priority inbox on the dashboard for students flagged with `needs_intake_approval`, with a collapsed summary header that splits pending counts into new (unassigned) and existing (assigned) students. It keeps loading/error/empty states visible with retry support, retains per-student collapsible sections, and the approval flow is wired to `/api/intake/approve` with agreement confirmation. Admins can filter by instructor/unassigned while seeing a streamlined single-row identity summary.
