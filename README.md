...
- **Intake Review Queue** (`src/features/dashboard/components/IntakeReviewQueue.jsx`) renders a high-priority inbox on the dashboard for students flagged with `needs_intake_approval`, with collapsible student sections and an approval flow wired to `/api/intake/approve` that requires an agreement confirmation. It shows the configured important fields, lets reviewers toggle the full intake, and admins can filter by instructor/unassigned while seeing a streamlined single-row identity summary.
