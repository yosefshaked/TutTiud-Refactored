...
- **Intake Review Queue** (`src/features/dashboard/components/IntakeReviewQueue.jsx`) renders a high-priority inbox on the dashboard for students flagged with `needs_intake_approval`, with a one-click approval action wired to `/api/intake/approve` and display labels sourced from `intake_display_labels`.
...
- **Intake settings** (`src/components/settings/IntakeSettingsCard.jsx`) lets admins map Microsoft Forms source keys, edit auto-learned question labels, and rotate the `external_intake_secret` that authenticates `/api/intake` calls.
...
- `POST /api/intake` – public intake endpoint for Power Automate; requires `x-org-id` + `x-intake-secret` headers, auto-learns question labels from the form definition, and writes intake history + approval flag.
