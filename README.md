...
- **Intake Review Queue** (`src/features/dashboard/components/IntakeReviewQueue.jsx`) renders a high-priority inbox on the dashboard for students flagged with `needs_intake_approval`, with a collapsed summary header that splits pending counts into new (unassigned) and existing (assigned) students using large-number tiles for quick scanning. It keeps loading/error/empty states visible with retry support, retains per-student collapsible sections, and the approval flow is wired to `/api/intake/approve` with agreement confirmation. Admins can filter by instructor/unassigned while seeing a streamlined single-row identity summary.
...
- `POST /api/intake/approve` – authenticated approval endpoint that clears `needs_intake_approval` and records `metadata.last_approval` with agreement metadata.
