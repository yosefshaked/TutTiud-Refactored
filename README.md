...
- **ComplianceHeatmap** (`src/features/dashboard/components/ComplianceHeatmap.jsx`) draws the weekly board using `/api/weekly-compliance`, keeps the SessionListDrawer drill-down per cell, and now wires the "תצוגה מפורטת" action to `/api/daily-compliance` so a modal day breakdown appears after a short loading state.
- **DayDetailView** (`src/features/dashboard/components/DayDetailView.jsx`) is the modal timeline for a single day: it receives the API payload as `dayData`, shows the localized date/summary header, renders each hour with its time slots, and colors every student card by instructor with ✔/✖ status icons.
