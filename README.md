...
- **Keyboard save shortcut** – all forms wire the `useKeyboardSubmit` hook (`src/hooks/useKeyboardSubmit.js`) to encourage intentional saves. Pressing Enter alone shows a Hebrew tip toast (“טיפ: יש ללחוץ על Ctrl + Enter לשמירה, או Tab למעבר בין שדות.”) without blocking the browser’s default behavior. Ctrl/Cmd + Enter triggers the save handler, and Tab navigation keeps working.
