...
- **Intake Review Queue** (`src/features/dashboard/components/IntakeReviewQueue.jsx`) renders a high-priority inbox on the dashboard for students flagged with `needs_intake_approval`, surfaced first as a scorecard-style summary with big-number tiles for new (unassigned) and existing (assigned) students. Clicking each tile (or the "פתח תור" action) opens the queue with the appropriate filter, while loading/error/empty states remain visible with retry support. The detailed list preserves per-student collapsible sections and approval flow via `/api/intake/approve` with agreement confirmation. Admins can still filter by instructor/unassigned while seeing a streamlined single-row identity summary.
