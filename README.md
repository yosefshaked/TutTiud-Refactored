...
- **Keyboard save shortcut** – all forms wire the `useKeyboardSubmit` hook (`src/hooks/useKeyboardSubmit.js`) to block accidental Enter submissions. Pressing Enter alone shows a Hebrew tip toast (“טיפ: יש ללחוץ על Ctrl + Enter לשמירה, או Tab למעבר בין שדות.”); Ctrl/Cmd + Enter triggers the save handler, and Tab navigation keeps working.
