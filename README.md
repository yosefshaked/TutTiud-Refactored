...
- **Intake Review Queue** (`src/features/dashboard/components/IntakeReviewQueue.jsx`) renders a high-priority inbox on the dashboard for students flagged with `needs_intake_approval`, with a one-click approval action wired to `/api/intake/approve`.
...
- **Intake settings** (`src/components/settings/IntakeSettingsCard.jsx`) lets admins map Microsoft Forms source keys and rotate the `external_intake_secret` that authenticates `/api/intake` calls.
