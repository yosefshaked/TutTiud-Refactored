...
- **NewSessionModal.jsx** – orchestrates data dependencies: loads the student roster (admin vs. instructor scope) while honoring the inactive visibility setting, fetches `session_form_config`, and surfaces loading/error states. On submit it posts to `/api/sessions` with `{ student_id, date, service_context, content }`, keeps the save action disabled until all required fields (including the date) are satisfied, and triggers any supplied `onCreated` callback before closing.
- **NewSessionForm.jsx** – Hebrew UI for the session questionnaire. It pre-selects the active student when invoked from the detail page, shows each student’s default day/time beside their name, pre-fills the service field, mirrors the Active / Inactive filter so dropdowns stay focused on current students, and collects answers for every configured question (text, textarea, select, radio/button groups, numeric fields, and range scales). The session date now starts blank, forces an explicit date selection before submission, and empty responses are stripped before sending the payload.
