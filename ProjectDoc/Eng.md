...
- Keyboard submission guard: the reusable `useKeyboardSubmit` hook (`src/hooks/useKeyboardSubmit.js`) keeps Enter usable while nudging users toward Ctrl/Cmd + Enter. It shows a Hebrew tip toast (“טיפ: יש ללחוץ על Ctrl + Enter לשמירה, או Tab למעבר בין שדות.”) on plain Enter without blocking default behavior, and triggers the provided `onSave` handler only when users press Ctrl/Cmd + Enter. Attach it to each form via `onKeyDown`, and disable it (`isEnabled=false`) while saving or after success to avoid duplicate toasts. Tab navigation stays unchanged, and the hook skips click-intent targets (buttons/links) so native Enter activation still works.
