...
- Keyboard submission guard: the reusable `useKeyboardSubmit` hook (`src/hooks/useKeyboardSubmit.js`) blocks plain Enter on forms, shows a Hebrew tip toast (“טיפ: יש ללחוץ על Ctrl + Enter לשמירה, או Tab למעבר בין שדות.”), and triggers the provided `onSave` handler only when users press Ctrl/Cmd + Enter. Attach it to each form via `onKeyDown`, and disable it (`isEnabled=false`) while saving or after success to avoid duplicate toasts. Tab navigation stays unchanged.
