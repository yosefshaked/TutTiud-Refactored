...
- **Legacy import attribution:** Imported rows mirror standard session metadata. `metadata` records `created_by`, `created_role`, and `source='legacy_import'`, always. When the CSV structure matches the current questionnaire the handler also stamps `form_version`; for custom-structure uploads the version is omitted to avoid implying schema alignment. Each row sets `instructor_id` from the student's assigned instructor. Uploads fail with `student_missing_instructor` when no assignment exists.
