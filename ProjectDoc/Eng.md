...
| `/api/intake` | POST | External Robot | Public endpoint for Microsoft Forms intake via Power Automate. Requires `x-org-id` plus `x-intake-secret` (validated against `external_intake_secret`), maps incoming fields using `intake_field_mapping`, and writes `intake_responses` + `needs_intake_approval` from the raw payload. |
...
- Intake Bridge uses `intake_field_mapping` + `external_intake_secret` in `tuttiud.Settings` to accept Microsoft Forms payloads, relies on a manually maintained `intake_display_labels` dictionary for question labels, and routes approvals through `/api/intake/approve`, keeping intake decisions explicit.
