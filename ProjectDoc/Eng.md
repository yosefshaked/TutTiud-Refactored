...
| `/api/students-list` | GET | All Users | Unified endpoint; admins see all students, non-admins filtered by `assigned_instructor_id`. Returns active students by default (`status=active`), with `status=inactive` and `status=all` options plus `include_inactive=true` for legacy callers. Dismissed intakes are excluded unless `include_dismissed=true` is provided. Responses echo the `is_active` flag so the UI can render lifecycle state. Replaces legacy `/api/students` and `/api/my-students` endpoints. |
