...
- **Legacy import backend (`POST /api/students/{id}/legacy-import`):** Admin/Owner-only endpoint that checks `can_reupload_legacy_reports` before allowing replacements. When uploads are permitted it purges prior `is_legacy` rows for the student, then ingests the submitted CSV (expects JSON body with `csv_text`, `structure_choice`, `session_date_column`, and either `column_mappings` or `custom_labels`) and writes new `SessionRecords` rows flagged with `is_legacy=true`.
