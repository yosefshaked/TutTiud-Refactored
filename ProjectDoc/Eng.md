...
| `/api/intake` | POST | External Robot | Public endpoint for Microsoft Forms intake via Power Automate. Validates `x-intake-secret` against `external_intake_secret`, maps incoming fields using `intake_field_mapping`, and writes `intake_responses` + `needs_intake_approval`. |
