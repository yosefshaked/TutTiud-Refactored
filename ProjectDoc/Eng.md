...
| `tuttiud."Settings"` | JSON configuration bucket per tenant. | `id`, `key` (unique), `settings_value` (includes `intake_field_mapping`, `external_intake_secret`, and `student_tags`) |
...
| `/api/intake` | POST | External Robot | Public endpoint for Microsoft Forms intake via Power Automate. Requires `x-org-id` plus `x-intake-secret` (validated against `external_intake_secret`), maps incoming fields using `intake_field_mapping`, and writes `intake_responses` + `needs_intake_approval`. |
| `/api/intake/approve` | POST | Admin/Owner/Instructor | Approves a pending intake by setting `needs_intake_approval=false` and appending `metadata.last_approval` (timestamp + approver). Members may only approve their assigned students. |
...
- Intake Bridge uses `intake_field_mapping` + `external_intake_secret` in `tuttiud.Settings` to accept Microsoft Forms payloads and routes approvals through `/api/intake/approve`, keeping intake decisions explicit.
