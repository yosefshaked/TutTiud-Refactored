...
- מגן קיצורי מקלדת: ההוק `useKeyboardSubmit` (`src/hooks/useKeyboardSubmit.js`) חוסם הקשה על Enter בטפסים, מציג טוסט טיפ בעברית ("טיפ: יש ללחוץ על Ctrl + Enter לשמירה, או Tab למעבר בין שדות."), ומפעיל את ה-handler של `onSave` רק כשנלחץ Ctrl/Cmd + Enter. חברו אותו ל-`onKeyDown` של כל טופס והשביתו אותו (`isEnabled=false`) בזמן שמירה או אחרי הצלחה כדי למנוע טוסטים כפולים. ניווט עם Tab נשאר זהה.
