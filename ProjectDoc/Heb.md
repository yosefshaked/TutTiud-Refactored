...
1. **שליחת הזמנה בפונקציית Azure (`POST /api/invitations`)** – מנהלי המערכת מזניקים אימייל הזמנה שמטמיע בכתובת ההפניה את `token_hash` של Supabase ואת `invitation_token` של בקרת הארגון.
   - המטפל מאמת קיום משתמש באמצעות `supabase.auth.admin.getUserByEmail(email)` ומתייחס ל-"User not found" (או סטטוס ‎404) כהוכחה לכך שאין חשבון קיים.
...
- **DashboardPage.jsx** – נמצא ב-`src/pages/DashboardPage.jsx`, מציג קבלת פנים מותאמת למשתמש המחובר ושתי פעולות מודגשות:
  מעבר ל-`/my-students` ופתיחה של חלון רישום מפגש חדש באמצעות כפתור ה-+ המרכזי.
...
- **StudentDetailPage.jsx** – נתיב חדש `/students/:id` הזמין למנהלים ולמדריכים. טוען את התלמיד המתאים דרך נקודת ה-API הרלוונטית, מציג פרטי קשר והגדרות תזמון ומציג היסטוריית מפגשים עם טיפול במצב בו נקודת הקצה עדיין לא זמינה.
...
- **NewSessionModal.jsx** – מנהלת את תלותי הנתונים: טוענת את רשימת התלמידים לפי תפקיד המשתמש, מושכת את `session_form_config`, מציגה מצבי טעינה/שגיאה ושולחת בקשה אל `/api/sessions` עם השדות `{ student_id, date, service_context, content }`, תוך הפעלת `onCreated` אם הועבר.
- **NewSessionForm.jsx** – טופס בעברית לשאלון המפגש. בוחר תלמיד אוטומטית כאשר נפתח מעמוד הפרטים, מציג ליד כל תלמיד את יום/שעת ברירת המחדל, ממלא מראש את השירות ומאפשר להשיב לכל סוג שאלה שהוגדר (טקסט, טקסט ארוך, בחירה מרשימה, כפתורי בחירה, שדות מספריים וסולם מדורג). ערכים ריקים נמחקים מהמטען לפני השליחה.
...
- **קטלוג תגיות תלמידים** – התגיות הארגוניות נשמרות בשורת `student_tags` בטבלת `tuttiud."Settings"`. נקודת הקצה `GET /api/settings/student-tags` זמינה לכל חבר בארגון הפעיל ומחזירה מערך תגיות מנורמל, בעוד `POST /api/settings/student-tags` (מנהלים/בעלי ארגון בלבד) מוסיפה תגית חדשה עם מזהה UUID ייחודי. בצד הלקוח משתמשים ב-`useStudentTags()` (`src/features/students/hooks/useStudentTags.js`) ובמרכיב `StudentTagsField.jsx` להצגת הבחירה והמודל במסכי יצירת/עריכת תלמידים. העמודה `Students.tags` שומרת מערך מזהים וכך נותרת תאימה לאחור למחרוזות ההיסטוריות.
...
- **התאמה בזרימת הרישום** – `NewSessionForm.jsx` תומך במגוון סוגי השאלות, מציג רשימות בחירה, כפתורי רדיו/כפתורים, שדות מספר וסולם מדורג לצד הטקסטים החופשיים, ודואג לאכוף שדות חובה בצד הלקוח.
