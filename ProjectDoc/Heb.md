...
- מגן קיצורי מקלדת: ההוק `useKeyboardSubmit` (`src/hooks/useKeyboardSubmit.js`) משאיר את Enter זמין אבל מכוון לשמירה מכוונת עם Ctrl/Cmd + Enter. הוא מציג טוסט טיפ בעברית ("טיפ: יש ללחוץ על Ctrl + Enter לשמירה, או Tab למעבר בין שדות.") על Enter רגיל בלי לחסום את ברירת המחדל של הדפדפן, ומפעיל את ה-handler של `onSave` רק כשנלחץ Ctrl/Cmd + Enter. חברו אותו ל-`onKeyDown` של כל טופס והשביתו אותו (`isEnabled=false`) בזמן שמירה או אחרי הצלחה כדי למנוע טוסטים כפולים. ניווט עם Tab נשאר זהה, וההוק מדלג על יעדי הקלקה (כפתורים/קישורים) כדי שלא לפגוע בהפעלה טבעית עם Enter.
