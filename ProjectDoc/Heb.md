...
- **תצוגת ציות שבועית** – `src/features/dashboard/components/WeeklyComplianceView.jsx` מציגה לוח שבועי מותאם-תפקיד שמוזן מ-`/api/weekly-compliance`. השרת מחשב מראש את נתוני השבבים, מצמצם את חלון השעות לשעות הפעילות ומסמן כל מפגש עבר או מפגש של היום ללא תיעוד ב-✖, כך שקל לזהות חוסרים. בדסקטופ מוצג השבוע המלא ואילו במובייל נבחר יום בודד עם אותם שבבים והגדרות אינטראקציה. כותרות הימים מתורגמות לעברית ומציגות את התאריך בשורה נפרדת לשמירה על אחידות. הגריד "בסגנון Outlook" בקפיצות של 30 דקות ממקם את השבבים בדיוק תת-חצי-שעה (לדוגמה :15 ו-:45 באמצע התא). מפגשים חופפים מוצגים זה לצד זה עד שני שבבים; מפגשים נוספים נספרים כתגית "+X נוספים" שממוקמת מיד מתחת לשורה הנראית ופותחת פופאובר עם רשימת כל התלמידים באותו חלון זמן, כולל קישורים ישירים לפרופיל. ריחוף (בדסקטופ) או הקשה (במסך מגע) פותחים חלונית אינטראקטיבית עם שם התלמיד, המדריך, שעת המפגש וקישור "צפה בפרופיל". מדריכים לא פעילים (`is_active = false`) מקבלים טקסטורה מפוספסת על השבב כדי להבליט בעיות. מקרא המדריכים עבר לקומפוננטה עצמאית `src/features/dashboard/components/InstructorLegend.jsx` שמבצעת שליפה עצמאית, מוצגת ככרטיס דביק בעמודה שמאלית. `src/pages/DashboardPage.jsx` מסדר את לוח הבקרה ברשת דו-עמודתית בדסקטופ כך שהמקרא נמצא בעמודה הראשונה והלוח משתמש בכל הרוחב בעמודה השנייה; במסכים צרים המקרא נערם מעל הווידג'ט אך נשאר דביק בתוך העמודה בזמן הגלילה. סף מבוסס-רוחב של ‎1,015‎ פיקסלים מחליף אוטומטית לתצוגת יום (ומסתיר את מתג יום/שבוע) כדי למנוע עומס עמודות, בעוד רוחבים גדולים יותר חוזרים לברירת המחדל של תצוגת השבוע עם עמודת שעות מצומצמת ל-60 פיקסלים ומתאם תצוגה זמין תמיד. בכפתור היום הנבחר גם התאריך מקבל את צבע הטקסט הראשי כדי שלא ייעלם על רקע הסגול.
