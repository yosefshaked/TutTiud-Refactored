...
- גשר הקליטה (Intake Bridge) משתמש ב-`intake_field_mapping` וב-`external_intake_secret` מתוך `tuttiud.Settings`, ממפה לפי טקסט השאלה בעברית כפי שמופיע ב-HTML, שומר את ה-HTML המקורי ב-`intake_responses.intake_html_source`, והאישורים מתבצעים דרך `/api/intake/approve` כדי לשמור על בקרה ידנית עם הצהרת הסכמה. בלוח הבקרה מוצגים שדות חשובים בכרטיסיות קליטה מתקפלות עם אפשרות להרחיב לצפייה מלאה.
