...
- **תצוגת ציות שבועית** – `src/features/dashboard/components/WeeklyComplianceView.jsx` מציגה לוח שבועי מותאם לתפקיד שמוזן מ-`/api/weekly-compliance`. השרת מחשב מראש את מקרא הצבעים, מקצר את טווח השעות לפי השעות הפעילות בארגון ומסמן כל מפגש עבר ב-✔ (הושלם) או ✖ (חסר). במחשב מוצג השבוע המלא, בעוד שבמובייל התצוגה מתכווצת ליום אחד עם אותם שבבים ומקרא צבעים. כותרות הימים תורגמו לעברית ומציגות את התאריך בשורה נוספת בכל מצב תצוגה כדי לשמור על אחידות. גריד הדסקטופ ארוז כעת במכל גלילה אופקית פנימית כך שהגלילה נשארת בתוך הווידג'ט ולא גורמת לכל הדף לקבל פס גלילה אופקי.
