...
| `/api/intake` | POST | רובוט חיצוני | נקודת קצה ציבורית לקליטת טפסי Microsoft Forms דרך Power Automate. דורשת את הכותרות `x-org-id` ו-`x-intake-secret` (מאומת מול `external_intake_secret`), ממפה שדות לפי `intake_field_mapping` וכותבת `intake_responses` + `needs_intake_approval` מתוך המטען הגולמי. |
...
- גשר הקליטה (Intake Bridge) משתמש ב-`intake_field_mapping` וב-`external_intake_secret` מתוך `tuttiud.Settings`, נשען על מילון ידני ב-`intake_display_labels`, והאישורים מתבצעים דרך `/api/intake/approve` כדי לשמור על בקרה ידנית.
