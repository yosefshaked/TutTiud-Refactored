...
- **תצוגת ציות שבועית** – `src/features/dashboard/components/WeeklyComplianceView.jsx` מציגה לוח שבועי מותאם לתפקיד שמוזן מ-`/api/weekly-compliance`. השרת מחשב מראש את מקרא הצבעים, מקצר את טווח השעות לפי השעות הפעילות בארגון ומסמן כל מפגש עבר ב-✔ (הושלם) או ✖ (חסר). במחשב מוצג השבוע המלא, בעוד שבמובייל התצוגה מתכווצת ליום אחד עם אותם שבבים ומקרא צבעים.
- **פעולות בלוח הבקרה** – `DashboardPage.jsx` עדיין מקדם את כרטיסי הניווט ל"התלמידים שלי" / "ניהול תלמידים" ולפתיחת רישום מפגש חדש. ווידג'ט הציות נטען רק לאחר שהחיבור למסד הנתונים זמין, ובינתיים מופיעה הודעת הסבר במקום הלוח.
