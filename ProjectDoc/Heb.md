...
- **ComplianceHeatmap** – `src/features/dashboard/components/ComplianceHeatmap.jsx` הוא הרכיב שמוצג בלוח הבקרה הראשי ומצייר את טבלת השעות-ימים על בסיס `/api/weekly-compliance`. הוא משמר את מגירת הפרטים (`SessionListDrawer`) לכל תא, מוסיף הודעות טעינה/שגיאה ברמת הכרטיס, וכעת גם מחבר כל כותרת יום לכפתור "תצוגה מפורטת" שמזניק בקשה אל `/api/daily-compliance`. לאחר שהנתונים מגיעים מוצג מודל `DayDetailView`, והמשתמש רואה בקלות אילו תלמידים תועדו או חסרים תיעוד באותו היום.
- **DayDetailView** – `src/features/dashboard/components/DayDetailView.jsx` הוא מודל עצמאי שמקבל את תוצאת `/api/daily-compliance` כ-`dayData`. הכותרת מציגה את התאריך בעברית יחד עם משפט תקציר (“X תלמידים מתוך Y מפגשים”), ומתחתיה נפרסת ציריית זמן אנכית לפי שעות. כל שעה מכילה את משבצות הזמן שבהן מתקיימים מפגשים, וכל משבצת מציגה כרטיס צבעוני לפי צבע המדריך עם שם התלמיד, שם המדריך ואייקון סטטוס (✔ עבור מתועד, ✖ כאשר חסר תיעוד, ונקודה ניטרלית עבור מפגשים עתידיים). הרכיב אינו מבצע קריאות רשת; השליטה והטעינה מנוהלות ב-`ComplianceHeatmap` בלבד.
