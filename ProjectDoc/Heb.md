...
- **עדכוני ניווט** – `AppShell.jsx` מחשבת את פריטי הניווט על סמך תפקיד החבר. מנהלים נשארים עם היעד `/admin/students`, בעוד
  שמדריכים מנותבים אל `/my-students`. הקובץ `src/main.jsx` חושף את הנתיב החדש כדי שמדריכים יגיעו לרשימת התלמידים המסוננת שלהם.

## 11. יסודות מערכת העיצוב (Mobile-First UI Kit)
