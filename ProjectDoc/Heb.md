...
- **יישור משופר** – שלב המיפוי משתמש בשתי כרטיסיות תואמות לבחירת עמודת תאריך ולשיוך שירות (`legacy-import-card` + `legacy-import-basic-grid`), כך שהכותרות לא נדחפות יותר לאותה שורה של בוחר התאריך.
- **קריאות כפתורים** – המיקרו-קופי הארוך בכפתורי בחירת המבנה/שירות עטוף עם `legacy-import-cta-button` כדי למנוע גלישות טקסט ולשמור על פוטר דביק ברור.
- **תצוגה מקדימה בהקשר** – טבלת התצוגה המקדימה מציגה גם את ערך התאריך המקורי וגם את התאריך המפוענח לצד שדות המיפוי, כך שהמשתמש רואה את התאריך בהקשר של כל שורה. החצים במסך האישור הותאמו לכיוון RTL (←).
