...
- **תצוגת ציות שבועית** – `src/features/dashboard/components/WeeklyComplianceView.jsx` מציגה לוח שבועי מותאם לתפקיד שמוזן מ-`/api/weekly-compliance`. השרת מחשב מראש את מקרא הצבעים, מקצר את טווח השעות לפי השעות הפעילות בארגון ומסמן כל מפגש עבר ב-✔ (הושלם) או ✖ (חסר). במחשב מוצג השבוע המלא, בעוד שבמובייל התצוגה מתכווצת ליום אחד עם אותם שבבים ומקרא צבעים. כותרות הימים תורגמו לעברית ומציגות את התאריך בשורה נוספת בכל מצב תצוגה כדי לשמור על אחידות. עדכון נובמבר 2025 מאמץ גריד "בסגנון Outlook" בקפיצות של 30 דקות – ה-API מחזיר כעת את `sessionDurationMinutes` וטווח זמן עם `intervalMinutes` של 30 דקות, והפרונט ממקם שבבים בדיוק תת-חצי-שעה (לדוגמה :15 ו-:45 מופיעים במרכז התא). מפגשים חופפים מוצגים זה לצד זה (עד שני שבבים במקביל) וכל העודפים נאספים לתגית "+X נוספים" שמוצמדת לתחתית המשבצת ומפתחת פופאובר עם כל התלמידים של אותו חלון זמן. ריחוף (בדסקטופ) או הקשה (במסך מגע) פותחים חלונית אינטראקטיבית עם שם התלמיד, המדריך, שעת המפגש וקישור "צפה בפרופיל". מדריכים שסומנו כלא פעילים (`is_active = false`) מקבלים טקסטורה מפוספסת על השבבים שלהם כדי להדגיש צורך במעקב. מקרא המדריכים מרחף כעת בפינה השמאלית-עליונה של הווידג'ט בדסקטופ, נצמד לגבולות הכרטיס, מחליק/דוהה בעדינות כשהוא מתנתק וחוזר לרשימה פנימית במובייל. סף מבוסס-רוחב של ‎1,015‎ פיקסלים ברמת המכל מחליף אוטומטית לתצוגת יום (ומסתיר את מתג יום/שבוע) כדי למנוע עומס עמודות, בעוד רוחבים גדולים יותר חוזרים לברירת המחדל של תצוגת השבוע עם עמודת שעות מצומצמת ל-60 פיקסלים ומתאם תצוגה זמין תמיד. בכפתור היום הנבחר גם התאריך מקבל את צבע הטקסט הראשי כדי שלא ייעלם על רקע הסגול.
