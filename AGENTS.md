...
- Legacy import rows include standard session attribution: `metadata` always stores `created_by`, `created_role`, and `source='legacy_import'`. When the CSV matches the current questionnaire the handler also writes `form_version`; custom-structure imports omit it to avoid implying schema alignment. Each row writes `instructor_id` from the student's assigned instructor. Uploads fail with `student_missing_instructor` when the student lacks an assignment.
