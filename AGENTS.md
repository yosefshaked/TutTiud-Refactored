...
- **Settings keys** (tenant DB `tuttiud."Settings"`):
  ...
  - `intake_display_labels` stores auto-learned labels for form question IDs.
...
- **API endpoints**:
  - `POST /api/intake` is public (Power Automate) and requires `x-org-id` plus `x-intake-secret` before inserting/updating student intake data (auto-learns labels into `intake_display_labels`).
